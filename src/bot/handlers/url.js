...
import { getOrCreateChat } from '../../database/index.js';
import { Env } from '../../config/index.js';
import logger from '../../logger/index.js';

export function urlFilter(ctx) {
...
export async function urlHandler(ctx) {
  const message = ctx.message;
  const url = urlFromMessage(message);
  if (!url) return;

  if (hasHashtagEntity(message, 'skip')) return;
  ...
  try {
    await handleDownloadTask(ctx.api ? { api: ctx.api, botInfo: ctx.me } : ctx, ctx, extractorCtx);
  } catch (e) {
    handleError(ctx, ctx, extractorCtx, e);
  }
}

export function getChatFromCtx(ctx) {

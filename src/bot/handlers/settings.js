...
export async function settingsOptionsHandler(ctx) {
  ...
  switch (option) {
    ...
    case 'captions': {
      const keyboard = buildToggleKeyboard('captions', chat.captions, lang);
      await ctx.editMessageText(t('CaptionsSettingsMessage', lang), { reply_markup: keyboard });
      break;
    }
    case 'nsfw': {
      const keyboard = buildToggleKeyboard('nsfw', chat.nsfw, lang);
      await ctx.editMessageText(t('NsfwSettingsMessage', lang), { reply_markup: keyboard });
      break;
    }
    case 'silent': {
      const keyboard = buildToggleKeyboard('silent', chat.silent, lang);
      await ctx.editMessageText(t('SilentModeSettingsMessage', lang), { reply_markup: keyboard });
      break;
    }
    case 'media_album': {
      const keyboard = buildAlbumKeyboard(chat.mediaAlbumLimit, lang);
    ...
    case 'delete_links': {
      const keyboard = buildToggleKeyboard('delete_links', chat.deleteLinks, lang);
...
export async function settingsToggleHandler(ctx) {
  const data = ctx.callbackQuery?.data || '';
...
export async function settingsSelectHandler(ctx) {
  const data = ctx.callbackQuery?.data || '';
...
export async function settingsManyHandler(ctx) {
  const data = ctx.callbackQuery?.data || '';
...
function buildSettingsKeyboard(chat, lang, isGroup) {
  const onOff = v => (v ? `✅ ${t('EnabledButton', lang)}` : `❌ ${t('DisabledButton', lang)}`);
  const rows = [
...
function buildLanguageKeyboard(lang) {
  const langs = [
...
function buildToggleKeyboard(option, current, lang) {
  return {
    inline_keyboard: [
...
function buildAlbumKeyboard(current, lang) {
...
function buildExtractorsKeyboard(chat, lang) {

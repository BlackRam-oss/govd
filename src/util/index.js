...
import { MediaCodec, MediaType } from '../database/index.js';
...
export function extractBaseHost(rawURL) {
  try {
...
export function getNamedGroups(pattern, str) {
  const match = str.match(pattern);
  if (!match) return null;
  return { ...match.groups, match: match[0] };
}

export function unescapeURL(url) {
  return url.replace(/&amp;/g, '&');
}

// ── validation ────────────────────────────────────────────────────────────────

export function exceedsMaxFileSize(fileSize) {
  return fileSize > Env.MaxFileSize;
}

export function exceedsMaxDuration(duration) {
  return duration > Env.MaxDuration;
}

// ── codec parsing ─────────────────────────────────────────────────────────────

export function parseVideoCodec(codecs) {
...
export function parseAudioCodec(codecs) {
  const c = codecs.toLowerCase();
...
export function cleanupDownloads(ignoreTime = false) {
...
export function cleanupDownloadsJob() {
...
export function hashedError(err) {
...
export function urlFromMessage(message) {
...
export function hasHashtagEntity(message, hashtag) {
  const entities = message.entities || [];
...
export function getMessageFileId(message) {
...
export function ptr(v) { return v; }

...
import { TaskResult, DownloadedFormat } from '../models/index.js';
import { Errors, exceedsMaxFileSize, exceedsMaxDuration, BotError, hasHashtagEntity } from '../util/index.js';
...
export async function handleDownloadTask(bot, ctx, extractorCtx) {
  ...
  try {
    ...
    const caption = formatCaption(taskResult.media, bot.botInfo?.username || '', extractorCtx.chat.captions);
...
async function executeDownload(extractorCtx, isInline) {
  if (Env.Caching) {
    try {
...
async function taskFromDatabase(ctx) {
...
function checkAlbumLimit(n, chat) {
...
export function formatCaption(media, username, captionsEnabled) {

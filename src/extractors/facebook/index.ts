...
export const FacebookShareExtractor = new Extractor({
  ...
  async getFunc(ctx: ExtractorContext): Promise<{ media?: Media; url?: string }> {
    ...
    const finalUrl = (resp.request as any)?.res?.responseUrl || ctx.contentUrl;
    return { url: finalUrl };
  },
});
...
async function getVideoData(ctx: ExtractorContext): Promise<VideoData> {
...
async function scrapeVideoData(ctx: ExtractorContext): Promise<VideoData> {
...
function buildMedia(ctx: ExtractorContext, data: VideoData): Media {

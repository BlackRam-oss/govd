import { ExtractorContext, Extractor } from '../models/index.js';
...
const extractorsByHost: Map<string, Extractor[]> = buildExtractorsMap();

function buildExtractorsMap(): Map<string, Extractor[]> {
...
export function fromURL(url: string): { loop: () => Promise<ExtractorContext | null>; abortController: AbortController } {
  let currentURL = url;
  let redirectCount = 0;

  const abortController = new AbortController();

  const loop = async (): Promise<ExtractorContext | null> => {
    while (redirectCount <= MAX_REDIRECTS) {
      ...
      for (const e of candidates) {
        ...
        if (match) {
          extractor = e;
          groups = { ...match.groups as Record<string, string>, match: match[0] };
          break;
        }
      }
...
export async function resolveURL(url: string): Promise<ExtractorContext | null> {

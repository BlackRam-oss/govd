...
async function getVideoFromInv(ctx) {
  ...
  for (let i = 0; i < ctx.config.instance.length; i++) {
    try {
      ...
      lastErr = e;
      ctx.debug(`invidious instance failed: ${e.message}`);
    }
  }
  throw lastErr || new Error('all invidious instances failed');
}

async function getInvInstance(ctx, index) {
  return ctx.config.instance[index];
}

async function getFromInstance(ctx, instance) {
...
function parseInvFormats(data, instance) {
  const formats = [];

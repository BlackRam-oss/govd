...
export function t(messageId, lang = 'en', data = {}) {
  const msg = messages[messageId];
  if (!msg) return messageId;
  const template = msg[lang] || msg['en'] || messageId;
  return template.replace(/\{\{(\w+)\}\}/g, (_, key) => data[key] || `{{${key}}}`);
}

export function localizer(lang = 'en') {
  return {
    t: (messageId, data = {}) => t(messageId, lang, data),
  };
}
